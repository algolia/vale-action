...
class CheckRunner {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (annotations.length > 0) {
                ...
                if (this.getConclusion() == 'failure') {
                    ...
                    yield this.postBotComment(client, `Looks like there are some content linting errors. Please check the [files tab](${filesTabLink}) for more details.`, options);
                }
    ...
    successCheck(client, checkRunId, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const botCommentID = yield this.botCommentID(client, options);
            if (botCommentID > 0) { // We only update the bots message if it already posted one
                yield this.postBotComment(client, `Content linting passed 🎉`, options);
            }
            return;
        });
    }
    postBotComment(client, body, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const prNumber = this.getCurrentPRNumber();
            if (prNumber < 1) {
                return false;
            }
            const currentCommentID = yield this.botCommentID(client, options);
            ...
            const commentResponse = yield client.request(endpoint, {
                owner: options.owner,
                repo: options.repo,
    ...
    prefixBotMessage(body) {
        return `Beep-boop I'm the content linting bot 🤖\n\n ${body}`;
    }
    botCommentID(client, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const prNumber = this.getCurrentPRNumber();
            if (prNumber < 1) {
                return 0;
            }
            const currentComments = yield client.request(`GET /repos/${options.owner}/${options.repo}/issues/${prNumber}/comments`, {
                owner: options.owner,
                repo: options.repo,
                issue_number: prNumber,
            });
    ...
    static makeAnnotation(name, alert) {
        ...
        if (alert.Link !== '') {
            message = `${message}\nFor more details: <a href="${alert.Link}">${alert.Link}</a>`;
        }
        let annotation = {
            path: name,
